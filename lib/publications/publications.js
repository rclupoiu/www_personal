...
function createStudyElement(study) {
    ...
    function updateHeights() {
        ...
        const buttonsHeight = buttonsDiv.offsetHeight;
        const copyButtonHeight = copyButton.offsetHeight;

        // Update other elements or styles based on these heights
        studyContentDiv.style.marginBottom = `${bibtexHeight}px`;
        bibtexSection.style.top = `${textContentHeight}px`;
        copyButton.style.top = `calc(${bibtexHeight}px + ${textContentHeight}px - ${copyButtonHeight}px - 10px)`;
    }
